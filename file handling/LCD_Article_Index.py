...
newdf2 = pd.DataFrame([],columns = ['AISArticleIndex','LcdKeyDoc','KeyDoc','Subject','PrimarySubject','ISSUER_ID','KeyInstn','KeyInDocs'])
print (instnissuerdf)
print (newsissuerdf)
for x in range (len(articleIndexPrimary)):
    ...
    for y in range (len(newsissuerdf)):
        ...
        if e==a:
            newdf = newdf.append({'LcdKeyDoc': a, 'KeyDoc': b,'Subject':c,'PrimarySubject':d,'ISSUER_ID':f}, ignore_index=True)
    ...
    for j in range (len(newdf)):
     ...
     #    print (c)
      ...
      #   print (d)
         if g == p:
       #      print (a)
             newdf2 = newdf2.append({'LcdKeyDoc': l, 'KeyDoc': m,'Subject':n,'PrimarySubject':o,'ISSUER_ID':p,'KeyInstn':h,'KeyInDocs': k}, ignore_index=True)
...
for ab in range (len(newdf)):
    ...
    if ag not in instnissuerdf.values :
             newdf2 = newdf2.append({'LcdKeyDoc': ac, 'KeyDoc': ad,'Subject':ae,'PrimarySubject':af,'ISSUER_ID':ag}, ignore_index=True)
for ba in range (len(articleIndexPrimary)):
    ...
    if bc not in newdf.values :
             newdf2 = newdf2.append({'LcdKeyDoc': bc, 'KeyDoc': bd,'Subject':be,'PrimarySubject':bf,'ISSUER_ID':bg}, ignore_index=True)
